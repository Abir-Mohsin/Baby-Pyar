...
export default function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = err.code === 'auth/email-already-in-use' 
        ? 'এই ইমেইলটি ইতিমধ্যে ব্যবহার করা হয়েছে' 
        : (err.message || 'Error signing up');
      setError(errorMessage);
      toast.error('রেজিস্ট্রেশন ব্যর্থ হয়েছে: ' + errorMessage);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Remove the old 'if (submitted)' block if it was there
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4">
      ...
      >
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-black text-brand">অ্যাকাউন্ট তৈরি করুন</h1>
          <p className="text-gray-500 text-sm mt-1">নতুন অ্যাকাউন্ট তৈরি করতে সঠিক তথ্য দিন</p>
        </div>

        <form onSubmit={handleSignup} className="space-y-6">
          <div className="space-y-2">
            <label className="text-sm font-semibold text-gray-700 block ml-1">নাম</label>
          ...
          >
            {loading ? 'প্রসেসিং হচ্ছে...' : 'রেজিস্ট্রেশন'}
          </button>
        </form>
        ...
        <div className="mt-8 text-center text-gray-600">
          ...
          <Link to="/login" className="text-brand font-bold hover:underline">লগইন করুন</Link>
        </div>
      </motion.div>
    </div>
  );
